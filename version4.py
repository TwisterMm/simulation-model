...
class vehicle(object):
    ...
    def __str__(self):
        return "vehicle: %d arrival time: %.3f " % (self.name, self.time)
...
class traffic_intersection(object):
    ...
    def view_queue(self):        
        yield self.right_queue.get_queue()
...
def departure(env: Environment, traffic_light: traffic_intersection, red_light):
    global isRedLight
    while True:
        try:
            print("start moving")
            vehicle =  yield traffic_light.get()
            print(vehicle, "depart time: %.3f" % env.now)            
        except Interrupt:
...
def traffic_light(env: Environment, departure): 
    global isRedLight
    while True:
        try:
            print("\n\nred light: %.3f" % env.now)
            #red light
            if(isRedLight):                         
                departure.interrupt()
        except Interrupt:
            ...
            print("\n\ngreen light: %.3f" % env.now)
            isRedLight = True    

 
        

def green_light(env: Environment):
    yield env.timeout(T_GREEN)
...
if __name__ == "__main__":
    ...
    departure = env.process(departure(env, tr, red_light))
    red_light = env.process(traffic_light(env, departure))
